...
{
    el_t B; // blank element to return if not found
    Node* P = Front; // Start with the front of the list

    while (P != nullptr) // Traverse the list
    {
        if (P->Elem.getid() == X.getid()) // Compare id parts
        {
    ...
    return B; // Return blank element if not found
}
